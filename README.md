...
- 🩸 **MBTI / 血型** 字段，列表/卡片中带 ℹ️ 图标，**悬停即显示性格与健康小贴士**
- 🖼️ 每个联系人可**上传自定义头像照片**（PNG/JPG/WEBP，≤2MB），未上传时显示 emoji 头像
- 📌 **「纪念日」模块**：结婚、恋爱、入职、毕业…… 同样支持提前提醒与「已携手 N 年」统计；「即将到来」页合并展示生日与纪念日
- 🔔 可在生日前 **N 天**（如 1/3/7 天）多次提醒，每人可单独设置天数与渠道
- 📨 多渠道通知：**微信推送**（Server酱 / PushPlus / Bark）+ **飞书机器人** + 邮件（可选）
- 📝 **可编辑推送模板**：管理员可在「设置 → 消息模板」自定义生日/纪念日/测试通知的标题与正文，支持 `{name}`、`{days_until}`、`{age}` 等变量，保存后立即生效
- 👥 **多用户系统**：管理员 + 普通用户两种角色，首次访问引导创建管理员
- ⚙️ **全参数前台可调**：管理员在网页「设置」页即可修改所有参数，**每个参数均附作用说明**，保存后立即生效、无需重启容器
- 🎛️ **设置二级菜单**：设置页按「界面与外观 / 提醒策略 / 各渠道 / 消息模板 / 隐私与注册 / 系统」分组，左侧导航快速跳转
...
- 🔐 **每条记录三级权限**：**私人 / 家庭 / 公开**，默认值可在设置中配置；列表中以标签区分
...
- 🎨 **5 套完全不同的主流 UI 风格主题**：内置 **Clean Modern（极简现代）/ Glassmorphism（玻璃拟态）/ Soft UI（新拟态）/ Bauhaus（包豪斯）/ Material You** 五套主题。每套主题在变量、字体栈、圆角体系、阴影层次、按钮/卡片/标签/输入框/侧边栏/导航/开关/复选框形态上均有独立的现代设计语言，绝不只是换色；在「偏好」页一键切换、立即生效。界面支持 **简体中文 / 繁體中文 / English / 한국어 / 日本語** 五种语言，所有按钮、字段、表单与提示实时翻译，语言与主题各自保存在浏览器本地（按用户偏好生效）
...
每条生日与纪念日记录都有一个**可见性**字段，编辑时可在右下角选择；默认值在「设置 → 隐私与注册」中配置。

| 可见性 | 谁能看到 | 典型用途 |
|--------|----------|----------|
| **私人**（默认） | 仅记录所有者本人 | 不想让他人知道的私密日期 |
| **家庭** | 所有者 + 已接受其家庭邀请的成员 | 家人之间共享的生日 / 纪念日 |
| **公开** | 所有登录用户 | 团队 / 社群共用的公共纪念日 |

> 「公开」与「家庭」记录对其他用户**只读**；只有所有者本人可以编辑或删除。列表与卡片中以彩色标签（🔒 私人 / 🏠 家庭 / 🌐 公开）区分。

### 🏠 家庭共享

1. 在左侧「**家庭**」页点击「**+ 创建家庭**」即可建立家庭（你为家庭创建者）。
2. 在家庭详情中点「**邀请成员**」，输入对方的**用户名**发送邀请；对方会在「家庭」页顶部看到待处理邀请与红点提醒。
3. 对方点击「**接受**」后正式加入，此后所有标记为「**家庭**」的生日与纪念日会**自动出现在双方的家庭共享视图中**。
4. 待处理（未接受）的邀请会显示在邀请卡片中，创建者可随时撤销。

### 🔓 自助注册与密码找回

- **自助注册**：管理员在「设置 → 隐私与注册」中打开「**允许注册**」后，登录页会出现「注册」入口，任何人可自助建号（普通用户角色）。
- **忘记密码（自己）**：登录页点「**忘记密码**」，填写账号邮箱；若系统已配置邮件，会收到重置链接，点开即可设置新密码。未配置邮件时页面会提示联系管理员。
- **管理员重置**：管理员在「用户管理」中可对任意用户点「**重置密码**」，直接设置新密码（无需旧密码、无需邮件）。
- 重置密码会使该用户的旧登录令牌**立即失效**，需重新登录。
...
镜像在每次推送 `main` 时由 GitHub Actions 自动重建并发布到 `:latest`；每次发布 **GitHub Release（形如 `v2.6.5`）** 时会额外构建并推送**版本固定标签** `:v2.6.5`，方便你锁定版本。日常升级只需拉取新镜像：
...
```bash
docker pull ghcr.io/marcusyean/birthday-keeper:v2.6.5
```
...
- **忘记管理员密码？** 优先在登录页点「**忘记密码**」走邮件重置；若未配置邮件，可由其他管理员在「用户管理 → 重置密码」直接重设。⚠️ 真要重建账号时，删除 `./data/birthday.db` 中的用户（或整库删除后重建）会重新进入初始化引导，但**整库删除会连生日数据一起清空，务必先备份 `./data`**。
