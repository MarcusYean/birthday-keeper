...
- 📱 **移动端友好**：手机上导航自动变为底部标签栏，多列表格转为竖向卡片，弹窗从底部滑出全宽面板，工具栏与设置二级菜单自适应铺满，窄屏（≤420px）进一步压缩间距
- 📊 联系人视图增强：自动显示**下次生日 / 倒计时 / 年龄 / 已活天数**；支持列表 / 卡片 / 紧凑视图，支持按名称/倒计时/年龄/创建时间排序，支持按关系/性别/星座/生肖/历法/生日月份分组；**每页数量可自定义**
- ✅ **批量测试**：联系人列表每行可勾选，工具栏提供「全选 / 反选 / 取消选择」，勾选后点「🔔 批量测试」即可发送测试通知，无需在弹窗中翻找大量联系人
...
- 在联系人列表中勾选若干记录（工具栏「全选 / 反选 / 取消选择」可快速选择），再点击「🔔 批量测试」发送测试通知，渠道未配置时会返回明确的失败原因；纪念日页同理。
...
- **收不到通知？** 先在联系人列表勾选目标人员，点「🔔 批量测试」看返回信息；再检查设置页对应渠道是否「启用」、Token/Webhook 是否正确。
