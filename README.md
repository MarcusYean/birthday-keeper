...
- 🎛️ **设置二级菜单**：设置页按「界面与外观 / 提醒策略 / 各渠道 / 隐私与注册 / 系统」分组，左侧导航快速跳转
...
镜像在每次推送 `main` 时由 GitHub Actions 自动重建并发布到 `:latest`；每次发布 **GitHub Release（形如 `v2.6.4`）** 时会额外构建并推送**版本固定标签** `:v2.6.4`，方便你锁定版本。日常升级只需拉取新镜像：
...
```bash
docker pull ghcr.io/marcusyean/birthday-keeper:v2.6.4
```
