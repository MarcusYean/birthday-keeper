# -*- coding: utf-8 -*-
"""v2.5 端到端冒烟测试（依赖已启动的本地服务，默认 http://127.0.0.1:8111）：
初始化 → 登录 → 权限 → 生日 CRUD（含扩展/MBTI/血型）→ 头像上传
→ 纪念日 CRUD → 批量测试 → 即将到来(合并) → 设置热更新 → 用户管理 → 静态资源。

注：v2.5 新增的「注册开关 / 三级权限 / 家庭共享 / 找回密码」等流程，
由同目录的 verify_v25.py（自包含、无需启动服务）以 TestClient 端到端覆盖。"""
import base64
import time
import requests
